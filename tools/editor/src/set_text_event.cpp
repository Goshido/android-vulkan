...
pbr::android::TextUIElement null ( false, nullptr, std::string_view {} );
pbr::windows::TextUIElement nullEXT ( false, nullptr, std::string_view {} );
...
void SetTextEvent::Execute () noexcept
{
    if ( std::holds_alternative<std::string_view> ( _text ) )
    {
        ...
        _elementEXT.SetText ( std::get<std::string_view> ( _text ) );
        return;
    }
    ...
    _elementEXT.SetText ( std::get<std::u32string_view> ( _text ) );
}
