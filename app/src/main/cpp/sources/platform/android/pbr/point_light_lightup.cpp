...
#include <platform/android/pbr/point_light_descriptor_set_layout.hpp>
#include <platform/android/pbr/point_light_lightup.hpp>
#include <platform/android/pbr/point_light_pass.hpp>
...
{
    if ( !_program.Init ( renderer, renderPass, resolution ) ) [[unlikely]]
        return false;
...
) noexcept
{
    _program.SetLightData ( commandBuffer,
        transform,
        _descriptorSets[ std::exchange ( _itemReadIndex, ( _itemReadIndex + 1U ) % _descriptorSets.size () ) ]
    );
