...
constexpr char const* SHADER = "shaders/windows/gizmo_compose.cs.spv";

} // end of anonymous namespace

//----------------------------------------------------------------------------------------------------------------------

GizmoComposeProgram::GizmoComposeProgram () noexcept:
    ComputeProgram ( sizeof ( GizmoComposeProgram::PushConstants ) )
{
    // NOTHING
}

bool GizmoComposeProgram::Init ( VkDevice device, SpecializationData /*specializationData*/ ) noexcept
{
    ...
    AV_SET_VULKAN_OBJECT_NAME ( device, _pipeline, VK_OBJECT_TYPE_PIPELINE, "Gizmo compose" )
    return true;
}

void GizmoComposeProgram::Destroy ( VkDevice device ) noexcept
{
    ComputeProgram::Destroy ( device );
}

VkExtent3D GizmoComposeProgram::DispatchParams ( VkExtent2D const &resolution ) noexcept
{
    return
    {
...
VkPipelineShaderStageCreateInfo GizmoComposeProgram::InitShaderInfo ( std::vector<uint8_t> &cs,
    VkShaderModuleCreateInfo &moduleInfo,
